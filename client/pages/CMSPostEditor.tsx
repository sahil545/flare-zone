...
import type { WpPost } from "../../shared/wp";
...
export default function CMSPostEditor() {
  ...
  const [model, setModel] = useState<Partial<WpPost>>({
    title: { raw: "" },
    slug: "",
    status: "draft",
    content: { raw: "" },
  ...
  const save = async (publish = false) => {
    try {
      ...
      const saved = isNew
        ? await cms.createPost(payload)
        : await cms.updatePost(id!, payload);
      navigate(`/cms/blog/${saved.id}`);
    } catch (e) {
